import { View, Text, StyleSheet, Pressable, Image, Modal, Dimensions , TextInput, TouchableOpacity, Alert, Button, } from 'react-native'
...
import { GET_ALL_GASTOS, GET_GASTOS, GET_GASTOS_MONTH} from '../../../graphql/querys';
...
export default function ModalCreateGasto({ setModalVisible2, id, item }) {
  ...
  const [createGasto, { loading, error, data }] = useMutation(CREATE_GASTO, {
    refetchQueries:[{ query : GET_GASTOS, variables: { id: id }},{query: GET_GASTOS_MONTH, variables: { id: id } }],
    update(cache, { data }) {
        ...
        <KeyboardAwareScrollView
          ...
          <View style={styles.modalView}>
            ...
            {item ?
              ...
              >
                <Text style={{ color: 'white', fontSize: 18, fontWeight: "600" }}>Confirmar Cambios</Text>
              </Pressable>
              :
              <Pressable
                ...
                onPress={handleSubmit}
              >
                <Text style={{ color: 'white', fontSize: 18, fontWeight: "600" }}>{item ? 'Guardar Cambios' : 'Confirmar Gasto'}</Text>
              </Pressable>
            }
